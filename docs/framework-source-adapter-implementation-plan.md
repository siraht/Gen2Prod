...
| 2026-07-20 | Cumulative resource-pressure hardening | A full serial verification run exposed one synchronous Bun/Node `spawn` allocation failure after repeated browser, framework, and Docker dogfood. Docker process creation now retries only synchronous `EAGAIN`/`EBADF`/`EMFILE`/`ENFILE` allocation failures with a bounded backoff; an error emitted after a child starts is never retried | The previously failing project pipeline and Vue native-validation slices both pass independently; container and cumulative verification are rerun after the change |
...
| D63 | Stateful Docker commands may be retried only when OS process allocation fails synchronously | Retrying a command after a child exists could duplicate a container or network mutation, while a synchronous allocation exception proves the executable never started | Container commands make at most three allocation attempts with 25/50 ms backoff for descriptor/process exhaustion codes; child `error` events and command exit failures retain their original single-attempt semantics |
...
| L47 | Serial tests can still create cumulative OS pressure because each native framework validation owns subprocess, watcher, browser, and Docker lifecycles | Reproduce cumulative failures in isolation first, then harden only the resource-allocation boundary that is proven transient; never broaden retries to application or mutation failures |
