...
| 2026-07-20 | Cumulative test-runtime isolation | Full serial verification reproduced Bun `EBADF epoll_ctl` failures and later browser-work stalls only after unrelated integration files shared one long-lived runtime. The deterministic suite scheduler now discovers every test file, proves exact once-only coverage, keeps the fast unit corpus together, and gives every integration file a fresh synchronous runtime | All 76 discovered files pass in 30 fresh runtimes (177 unit plus 54 integration tests); pipeline, native Vue/SvelteKit/Astro, image research, convergence, container, and visual dirty/candidate/gold scoring all retain their ordinary assertions with no retry of production mutations |
| 2026-07-20 | Final plan completion audit | Re-read the implementation ledger and executable checklist after cumulative dogfood; no task remains unchecked, and no prose-only placeholder is counted as implementation | `rg '^\s*- \[ \]'` returns no results; typecheck, all isolated tests, acceptance, and production bundle are the terminal repository verification sequence |
| 2026-07-20 | Terminal repository verification | Ran the committed top-level verification command after the runtime-isolation correction | `bun run verify` exits 0: TypeScript passes; 177 unit and 54 integration tests pass across all 76 files; acceptance reports `ok:true`, 10 fixtures, 54 browser captures, visual recovery 1, mutation-control recall 1, and zero critical/content/semantic/accessibility/visual/BEM/drift/idempotence failures; the 150-module CLI production bundle succeeds |
...
| D63 | Test-runtime epoll corruption is contained with process isolation, never command retry | The reproduced `EBADF` can appear while awaiting a child that already has a PID, so retrying a stateful CLI/Docker command could duplicate real mutation | A deterministic synchronous parent launches bounded fresh Bun test runtimes; production command semantics remain single-attempt, and the scheduler proves all discovered files are assigned exactly once |
| D64 | An integration-test deadline must exceed the nested native-command deadline envelope | A 90-second outer deadline could kill a correct two-sandbox SvelteKit check/build/render sequence even though each authorized command remained inside its own contract deadline | Native framework dogfood receives a 180-second outer ceiling; command, preview-readiness, and process-shutdown timeouts remain independently strict and report the actual failing stage |
| D65 | Browser convergence gets an explicit outer harness deadline independent of its search budget | Bun's default five-second test ceiling can expire during browser startup under host pressure even when the bounded search performs zero iterations | The harness permits 30 seconds while `maxIterations: 2`, the approved loss threshold, and the assertion that a matching incumbent performs zero repair iterations remain unchanged |
...
| L47 | `--max-concurrency 1` serializes tests but does not reset a runtime's epoll state after subprocess, watcher, browser, and Docker lifecycles | Give every integration file a fresh runtime and retain strict single-attempt production commands; a later `EBADF` is not proof that an earlier stateful child never started |
| L48 | A fast isolated native test does not establish a safe outer deadline for a cumulative verification run | Size the outer harness timeout from the maximum authorized nested work, then use retained native-command evidence to distinguish slowness from failure instead of letting the harness erase diagnostics |
| L49 | Isolating only the integrations already known to be expensive leaves shorter browser tests vulnerable to pressure accumulated by their shared group | Discover integration files rather than maintaining a handpicked allowlist, assign each exactly once, and make process isolation the uniform integration-test contract |
