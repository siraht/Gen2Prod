...
The repository now runs all three loops for canonical HTML/compiled-CSS and its React, Vue, Svelte, Astro, WordPress, and Bricks output adapters, and adds a bounded strict image-only evidence path. Hash-bound screenshots feed deterministic segmentation, local OCR, semantic/content hypotheses, BEM HTML/SCSS emission, browser image diff, explicit interaction uncertainty, project-isolated one-change research, idempotence replay, and accepted/rejected trajectory distillation. A paired synthetic curriculum scores both dirty-to-target and candidate-to-target renders, so source cleanliness cannot hide visual regression. Canonical and adapter policy promotion is sealed behind holdouts and exact replay; requested interventions must actually execute; threshold activation is withheld until independent benchmark coverage is sufficient; and distilled data is evidence-deduplicated, group-isolated and contradiction-quarantined before runtime shadow/active use.

The adapter sub-loop treats canonical G2P-NF as the invariant program and framework serialization policy as the research surface. Each candidate is actually compiled and server-rendered with its target toolchain, compared structurally and by browser pixels to canonical HTML, and scored for component boundaries, native metadata, verified interactions, review burden, compute, and source size. The current promoted incumbent was learned from an intentionally weak baseline: BEM-block componentization and framework-native metadata were kept, a source-ineffective interaction simplification was reverted, and the sealed holdout passed with exact output-hash replay. Adapter trajectories can be blended through `distill --adapter`.
