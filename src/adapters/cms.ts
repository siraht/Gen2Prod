...
function cmsNode(node: PlannedNode, parentId: string | null): CmsNode {
  return {
    id: node.nodeId,
    parentId,
    tag: node.tag,
    ...
    children: node.children.map((child) => cmsNode(child, node.nodeId)),
  };
}

export function buildCmsDocument(compiled: CompiledPage, vendor: "wordpress" | "bricks"): CmsDocument {
  ...
  return CmsDocumentSchema.parse({
    ...
    stylesheet: "page.css",
    root: cmsNode(compiled.plan.semantics.root, null),
