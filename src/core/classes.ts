...
export function isUtilityClass(className: string): boolean {
  ...
  return /^u-\d+$/.test(candidate) || CONTENT_ALIGNMENT_UTILITY.test(candidate) || UTILITY_FAMILY.test(candidate) || /^-?(?:m|p|inset|top|right|bottom|left|translate-[xy])-/.test(candidate);
}
