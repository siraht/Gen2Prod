...
import { groupIsolatedSplit } from "./split.ts";
...
function trainSelector(trajectories: Trajectory[]): SelectorModel {
  const parts = groupIsolatedSplit(trajectories);
  ...
  return SelectorModelSchema.parse({ schemaVersion: "0.1.0", kind: "pass-selector", trainedAt: new Date().toISOString(), examples: trajectories.length, actions, defaultRanking, evaluation: { trainUtility: mean(parts.train), holdoutUtility: mean(parts.holdout), holdoutExamples: parts.holdout.length, trainGroups: parts.trainGroups.length, holdoutGroups: parts.holdoutGroups.length, groupLeakage: parts.leakageGroups.length } });
}
...
function trainVerifier(trajectories: Trajectory[]): VerifierModel {
  ...
  return VerifierModelSchema.parse({ schemaVersion: "0.1.0", kind: "candidate-verifier", trainedAt: new Date().toISOString(), examples: trajectories.length, rule: { maxHardGateFailures: 0, maxUnaccountedDeclarations: 0, requireMutationControls: true, requireIdempotence: true }, evaluation: { accuracy: (truePositive + trueNegative) / Math.max(rows.length, 1), precision: truePositive / Math.max(truePositive + falsePositive, 1), recall: truePositive / Math.max(truePositive + falseNegative, 1), holdoutExamples: rows.length, trainGroups: parts.trainGroups.length, holdoutGroups: parts.holdoutGroups.length, groupLeakage: parts.leakageGroups.length } });
}
...
function trainPlanner(trajectories: Trajectory[]): PlannerModel {
  ...
  return PlannerModelSchema.parse({ schemaVersion: "0.1.0", kind: "structured-planner", trainedAt: new Date().toISOString(), examples: trajectories.length, observationBuckets, vocabulary: { passes, evidenceActions }, evaluation: { actionCoverage: covered / Math.max(parts.holdout.length, 1), holdoutExamples: parts.holdout.length, trainGroups: parts.trainGroups.length, holdoutGroups: parts.holdoutGroups.length, groupLeakage: parts.leakageGroups.length } });
}
...
export async function distill(trajectoryPath: string | string[], outputDirectory: string, target: DistillTarget = "all"): Promise<DistillationResult> {
  ...
  const result = { dataset: {
    trajectories: datasets.trajectories.length,
    supervised: datasets.supervised.length,
    preferences: datasets.preferences.length,
    verifier: datasets.verifier.length,
    groups: datasets.audit.groups,
