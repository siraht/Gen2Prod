...
export async function prepareProjectCurriculum(options: PrepareProjectCurriculumOptions): Promise<ProjectSyntheticManifest> {
  ...
  for (const starter of STARTERS) for (const base of archetypes) for (let variant = 0; variant < variants; variant += 1) {
    ...
    await Promise.all([writeProject(goldRoot, fixtureId, goldSource, goldScss, true), writeProject(dirtyRoot, fixtureId, dirtySource, dirtyScss, false)]);
...
async function writeProject(root: string, name: string, source: string, scss: string, clean: boolean): Promise<void> {
  await Promise.all([
    writeJsonAtomic(join(root, "package.json"), { name, private: true, scripts: { build: "bun build ./src/App.tsx --outdir ./dist --external react" }, dependencies: { react: "19.0.0", vite: "7.0.0" } }),
