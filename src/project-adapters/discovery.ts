...
function profiles(dependencies: Record<string, string>, files: Set<string>): { profile: ProjectFrameworkProfile; target: ProjectContract["framework"]["target"]; evidence: string[] }[] {
  ...
  else if (files.has("src/App.tsx") || files.has("src/App.jsx") || (dependencies.react && !dependencies.astro)) found.push({ profile: dependencies.vite ? "react-vite" as const : "react-generic" as const, target: "react" as const, evidence: [dependencies.react ? `react:${dependencies.react}` : "react-entry"] });
  if (dependencies.nuxt) found.push({ profile: "nuxt" as const, target: "vue" as const, evidence: [`nuxt:${dependencies.nuxt}`] });
  else if (files.has("src/App.vue") || (dependencies.vue && !dependencies.astro)) found.push({ profile: "vue-vite" as const, target: "vue" as const, evidence: [dependencies.vue ? `vue:${dependencies.vue}` : "vue-entry"] });
  ...
  else if (files.has("src/App.svelte") || (dependencies.svelte && !dependencies.astro)) found.push({ profile: "svelte" as const, target: "svelte" as const, evidence: [dependencies.svelte ? `svelte:${dependencies.svelte}` : "svelte-entry"] });
