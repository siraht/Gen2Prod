...
export async function runProjectPipeline(input: ProjectPipelineInput): Promise<ProjectPipelineResult> {
  ...
  const replay = { schemaVersion: "0.1.0", runId, policyHash: input.policyHash, events: [event("project-inspect", [], [{ id: contractRef.id, sha256: contractRef.sha256 }], ["framework-source", "destination-build-contract"], "accepted", { filesInspected: source.files.length }), event("project-parse", [{ id: contractRef.id, sha256: contractRef.sha256 }], [{ id: sourceRef.id, sha256: sourceRef.sha256 }], ["framework-source"], source.unresolved.length ? "review" : "accepted", { nodes: source.roots.length, unresolved: source.unresolved.length }), event("project-plan", [{ id: sourceRef.id, sha256: sourceRef.sha256 }], [{ id: planRef.id, sha256: planRef.sha256 }], ["framework-source", "destination-path-ownership"], plan.requiredActions.some((item) => item.blocking) ? "review" : "accepted", { operations: plan.operations.length, predictedChangedBytes: plan.predictedChangedBytes }), event("project-sandbox", [{ id: planRef.id, sha256: planRef.sha256 }], [{ id: sandboxRef.id, sha256: sandboxRef.sha256 }], ["destination-path-ownership"], "accepted", { changedFiles: plan.predictedChangedFiles.length }, { kind: "inverse-patch", reference: sandboxRef.id }), event("project-validate", [{ id: sandboxRef.id, sha256: sandboxRef.sha256 }, { id: mutationRef.id, sha256: mutationRef.sha256 }], [{ id: validationRef.id, sha256: validationRef.sha256 }, { id: reportRef.id, sha256: reportRef.sha256 }], ["framework-source", "runtime-state-fixtures"], validation.accepted ? "accepted" : "rejected", { hardFailures: validation.hardFailures.length, mutationControlRecall: validation.mutationControlRecall, visualLoss: validation.metrics.visualLoss, sourceChurnBytes: validation.metrics.sourceChurnBytes }, { kind: "inverse-patch", reference: sandboxRef.id })], requiredActions: validation.requiredActions, manifestHash: hashJson({ contract: contractRef.sha256, source: sourceRef.sha256, plan: planRef.sha256, sandbox: sandboxRef.sha256, mutation: mutationRef.sha256, validation: validationRef.sha256, report: reportRef.sha256 }) };
  ...
  return { runId, artifactRoot, contract: discovery.contract, source, plan, validation, ...(baselineCapture ? { baselineCapture } : {}), ...(candidateCapture ? { candidateCapture } : {}), artifacts: { contract: contractRef.id, source: sourceRef.id, plan: planRef.id, sandbox: sandboxRef.id, mutation: mutationRef.id, validation: validationRef.id, report: reportRef.id, replay: replayRef.id } };
}
