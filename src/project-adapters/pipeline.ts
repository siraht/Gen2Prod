...
import { ProjectPatchPlanSchema, type ProjectAdapterPolicy, type ProjectCorrespondence, type ProjectPatchPlan, type ProjectRouteProjection } from "../schemas/project-adapters.ts";
...
import { startContainerProjectPreview } from "./container.ts";
import { projectRenderedRoutes } from "./projection.ts";
import { projectPolicyImpact } from "./policy-impact.ts";

type FixedPlanning = "root" | "contract" | "source" | "correspondence" | "canonicalOutputHash" | "policyHash" | "projectPolicy" | "mode" | "profile";
export type ProjectPipelineInput = {
  ...
  policyHash: string;
  projectPolicy?: ProjectAdapterPolicy | undefined;
  mode: Mode;
  profile: Profile;
...
export type ProjectPipelineResult = { runId: string; artifactRoot: string; contract: Awaited<ReturnType<typeof discoverProject>>["contract"]; source: Awaited<ReturnType<typeof parseProjectSource>>; plan: ProjectPatchPlan; validation: Awaited<ReturnType<typeof validateProjectPatch>>; baselineCapture?: CaptureResult; candidateCapture?: CaptureResult; projection?: ProjectRouteProjection; artifacts: { contract: string; source: string; policy?: string; plan: string; projection?: string; sandbox: string; mutation: string; validation: string; report: string; replay: string } };
...
export async function runProjectPipeline(input: ProjectPipelineInput): Promise<ProjectPipelineResult> {
  ...
  if (input.projectPolicy && hashJson(input.projectPolicy) !== input.policyHash) throw new Error("Configured project policy does not match the run request policy hash");
  ...
  const policyRef = input.projectPolicy ? await store.putJson("policy", { ...projectPolicyImpact(input.projectPolicy), policy: input.projectPolicy }, { id: `${runId}-policy`, producer: "project-policy", authorities: ["project-policy"] }) : undefined;
  const context = { ...input.planning, root: input.root, contract: discovery.contract, source, correspondence: input.correspondence, canonicalOutputHash: input.planning.canonicalOutputHash, policyHash: input.policyHash, ...(input.projectPolicy ? { projectPolicy: input.projectPolicy } : {}), mode: input.mode, profile: input.profile } satisfies ProjectPlanningContext;
  ...
  let projection: ProjectRouteProjection | undefined;
  let projectionRef: Awaited<ReturnType<ArtifactStore["putJson"]>> | undefined;
  if (input.previewUrl && discovery.contract.commands.preview) {
    ...
    await runSandboxCommands(baselineSandbox, discovery.contract, { ...(input.includeInstall ? { includeInstall: true } : {}), ...(input.containerImage ? { containerImage: input.containerImage } : {}) });
    baselineCapture = await captureSandbox(baselineSandbox, "baseline", discovery.contract, input);
    projection = await projectRenderedRoutes({ project: source, capture: baselineCapture, outputDirectory: join(baselineSandbox.artifactsRoot, "projection"), tokenRegistry: { schemaVersion: "dtcg-2025-10+gen2prod-0.1.0", conformsTo: ["DTCG Format Module 2025.10"], adapterSchema: "gen2prod-token-adapter-0.1.0", tokens: [] } });
    projectionRef = await store.putJson("project-route-projection", projection, { id: `${runId}-projection`, producer: "project-route-projection", inputs: [sourceRef.id], authorities: ["framework-source", "runtime-state-fixtures"] });
  }
  ...
  if (input.previewUrl && discovery.contract.commands.preview) {
    await runSandboxCommands(sandbox, discovery.contract, { ...(input.includeInstall ? { includeInstall: true } : {}), ...(input.containerImage ? { containerImage: input.containerImage } : {}) });
    candidateCapture = await captureSandbox(sandbox, "candidate", discovery.contract, input);
  }
  ...
  const secondPlan = await projectSourceAdapter(rediscovery.contract).planIntegration({ ...input.planning, root: sandbox.projectRoot, contract: rediscovery.contract, source: candidate, correspondence: input.correspondence, canonicalOutputHash: input.planning.canonicalOutputHash, policyHash: input.policyHash, ...(input.projectPolicy ? { projectPolicy: input.projectPolicy } : {}), mode: input.mode, profile: input.profile });
  const validation = await validateProjectPatch({ sandbox, contract: rediscovery.contract, source, candidate, plan, secondPlan, ...(baselineCapture ? { baselineCapture } : {}), ...(candidateCapture ? { candidateCapture } : {}), ...(input.targetCapture ? { targetCapture: input.targetCapture } : {}), registeredVariables: input.registeredVariables, ...(input.includeInstall ? { includeInstall: true } : {}), ...(input.containerImage ? { containerImage: input.containerImage } : {}), requireRuntime: true });
  const sandboxRef = await store.putJson("project-sandbox", { schemaVersion: "0.1.0", runId, sourceFingerprint: sandbox.sourceFingerprint, planId: plan.planId, outputHashes: Object.fromEntries(sandbox.prepared.outputFileHashes), isolationProof: sandbox.isolationProof ?? null, previewIsolationProof: sandbox.previewIsolationProof ?? null }, { id: `${runId}-sandbox`, producer: "project-sandbox", inputs: [planRef.id], authorities: ["destination-path-ownership"] });
  const mutationReport = await Bun.file(join(sandbox.artifactsRoot, "project-mutation-controls.json")).json();
  const mutationRef = await store.putJson("project-mutation-report", mutationReport, { id: `${runId}-mutation`, producer: "project-mutation-controls", inputs: [sandboxRef.id], authorities: ["framework-source", "runtime-state-fixtures"] });
  ...
  const projectionEvent = projectionRef ? [event("project-route-projection", [{ id: sourceRef.id, sha256: sourceRef.sha256 }], [{ id: projectionRef.id, sha256: projectionRef.sha256 }], ["framework-source", "runtime-state-fixtures"], "accepted", { states: projection!.states.length, dynamicRegions: projection!.states.reduce((sum, state) => sum + state.dynamicRegionIds.length, 0) })] : [];
  const policyEvent = policyRef ? [event("project-policy", [], [{ id: policyRef.id, sha256: policyRef.sha256 }], ["project-policy"], "accepted", { directives: projectPolicyImpact(input.projectPolicy!).directives.length })] : [];
  const replay = { schemaVersion: "0.1.0", runId, policyHash: input.policyHash, events: [...policyEvent, event("project-inspect", [], [{ id: contractRef.id, sha256: contractRef.sha256 }], ["framework-source", "destination-build-contract"], "accepted", { filesInspected: source.files.length }), event("project-parse", [{ id: contractRef.id, sha256: contractRef.sha256 }], [{ id: sourceRef.id, sha256: sourceRef.sha256 }], ["framework-source"], source.unresolved.length ? "review" : "accepted", { nodes: source.roots.length, unresolved: source.unresolved.length }), ...projectionEvent, event("project-plan", [{ id: sourceRef.id, sha256: sourceRef.sha256 }], [{ id: planRef.id, sha256: planRef.sha256 }], ["framework-source", "destination-path-ownership"], plan.requiredActions.some((item) => item.blocking) ? "review" : "accepted", { operations: plan.operations.length, predictedChangedBytes: plan.predictedChangedBytes }), event("project-sandbox", [{ id: planRef.id, sha256: planRef.sha256 }], [{ id: sandboxRef.id, sha256: sandboxRef.sha256 }], ["destination-path-ownership"], "accepted", { changedFiles: plan.predictedChangedFiles.length }, { kind: "inverse-patch", reference: sandboxRef.id }), event("project-validate", [{ id: sandboxRef.id, sha256: sandboxRef.sha256 }, { id: mutationRef.id, sha256: mutationRef.sha256 }], [{ id: validationRef.id, sha256: validationRef.sha256 }, { id: reportRef.id, sha256: reportRef.sha256 }], ["framework-source", "runtime-state-fixtures"], validation.accepted ? "accepted" : "rejected", { hardFailures: validation.hardFailures.length, mutationControlRecall: validation.mutationControlRecall, visualLoss: validation.metrics.visualLoss, sourceChurnBytes: validation.metrics.sourceChurnBytes }, { kind: "inverse-patch", reference: sandboxRef.id })], requiredActions: validation.requiredActions, manifestHash: hashJson({ contract: contractRef.sha256, source: sourceRef.sha256, ...(policyRef ? { policy: policyRef.sha256 } : {}), ...(projectionRef ? { projection: projectionRef.sha256 } : {}), plan: planRef.sha256, sandbox: sandboxRef.sha256, mutation: mutationRef.sha256, validation: validationRef.sha256, report: reportRef.sha256 }) };
  ...
  return { runId, artifactRoot, contract: discovery.contract, source, plan, validation, ...(baselineCapture ? { baselineCapture } : {}), ...(candidateCapture ? { candidateCapture } : {}), ...(projection ? { projection } : {}), artifacts: { contract: contractRef.id, source: sourceRef.id, ...(policyRef ? { policy: policyRef.id } : {}), plan: planRef.id, ...(projectionRef ? { projection: projectionRef.id } : {}), sandbox: sandboxRef.id, mutation: mutationRef.id, validation: validationRef.id, report: reportRef.id, replay: replayRef.id } };
}

async function captureSandbox(sandbox: Awaited<ReturnType<typeof createProjectSandbox>>, label: string, contract: Awaited<ReturnType<typeof discoverProject>>["contract"], input: ProjectPipelineInput): Promise<CaptureResult> {
  const preview = input.containerImage
    ? await startContainerProjectPreview({ root: sandbox.projectRoot, artifactsRoot: sandbox.artifactsRoot, contract, url: input.previewUrl!, image: input.containerImage, ...(input.previewEnvironment ? { environment: input.previewEnvironment } : {}) })
    : await startProjectPreview({ root: sandbox.projectRoot, contract, url: input.previewUrl!, ...(input.previewEnvironment ? { environment: input.previewEnvironment } : {}) });
  if (input.containerImage && "proof" in preview) sandbox.previewIsolationProof = preview.proof as Awaited<ReturnType<typeof startContainerProjectPreview>>["proof"];
  try { return await capturePage({ url: input.previewUrl!, outputDirectory: join(sandbox.artifactsRoot, label), viewports: [...new Set(contract.states.map((state) => state.viewport))], states: contract.states.map((state) => state.id), themes: [...new Set(contract.states.map((state) => state.theme))], stateFixtures: contract.states, ...(input.fixturePayloads ? { fixturePayloads: input.fixturePayloads } : {}), ...(input.browserExecutable ? { browserExecutable: input.browserExecutable } : {}), collectRenderedSource: true }); }
