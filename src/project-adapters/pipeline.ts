...
import { ProjectPatchPlanSchema, type ProjectCorrespondence, type ProjectPatchPlan, type ProjectRouteProjection } from "../schemas/project-adapters.ts";
...
export async function runProjectPipeline(input: ProjectPipelineInput): Promise<ProjectPipelineResult> {
  ...
  return { runId, artifactRoot, contract: discovery.contract, source, plan, validation, ...(baselineCapture ? { baselineCapture } : {}), ...(candidateCapture ? { candidateCapture } : {}), ...(projection ? { projection } : {}), artifacts: { contract: contractRef.id, source: sourceRef.id, plan: planRef.id, ...(projectionRef ? { projection: projectionRef.id } : {}), sandbox: sandboxRef.id, mutation: mutationRef.id, validation: validationRef.id, report: reportRef.id, replay: replayRef.id } };
}
