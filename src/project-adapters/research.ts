...
async function evaluated(evaluate: ProjectPolicyEvaluator, policy: ProjectAdapterPolicy, split: ProjectAdapterResearchEvaluation["split"], directory: string): Promise<ProjectAdapterResearchEvaluation> { await ensureDirectory(directory); const value = ProjectAdapterResearchEvaluationSchema.parse(await evaluate(policy, split, directory)); if (value.policyHash !== hashJson(policy)) throw new Error(`Evaluator returned a stale policy hash for ${split}`); const { telemetryHash, ...telemetry } = value.telemetry; if (hashJson(telemetry) !== telemetryHash) throw new Error(`Evaluator returned corrupt performance telemetry for ${split}`); const normalizedCost = telemetry.computeCost / telemetry.sampleCount, normalizedLatency = telemetry.wallTimeMs / telemetry.sampleCount / 1_000; if (Math.abs(value.fitness.normalizedCost - normalizedCost) > 1e-9 || Math.abs(value.fitness.normalizedLatency - normalizedLatency) > 1e-9) throw new Error(`Research resource fitness is not derived from retained telemetry for ${split}`); await writeJsonAtomic(join(directory, "evaluation.json"), value); return value; }
