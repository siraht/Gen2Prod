...
import type { ProjectContract, ProjectPatchPlan, SourceProject } from "../schemas/project-adapters.ts";
...
export async function runSandboxCommands(sandbox: ProjectSandbox, contract: ProjectContract, options: { includeInstall?: boolean; environment?: Record<string, string | undefined>; redactValues?: string[]; totalTimeoutMs?: number } = {}): Promise<ProjectCommandResult[]> {
  ...
  for (const command of commands) {
    const result = await runProjectCommand({ root: sandbox.projectRoot, contract, command, ...(options.environment ? { environment: options.environment } : {}), ...(options.redactValues ? { redactValues: options.redactValues } : {}), totalDeadlineAt: deadline });
    results.push(result);
