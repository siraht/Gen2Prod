...
export async function validateProjectPatch(input: ProjectValidationInput): Promise<ProjectValidationReport> {
  ...
  const handlerBindingsPreserved = bindingsRetained(input.source, input.candidate, new Set(["handler", "action"]));
  const dataBindingsPreserved = bindingsRetained(input.source, input.candidate, new Set(["data", "loader", "prop", "state", "store", "ref"]));
...
function domFacts(capture: CaptureResult) { const tags = new Set<string>(), text = new Set<string>(), urls = new Set<string>(), forms = new Set<string>(), interactions = new Set<string>(), classes = new Set<string>(); for (const condition of capture.captures) for (const raw of condition.dom as { tag?: string; text?: string; contentText?: string; attributes?: Record<string, string> }[]) { if (raw.tag) tags.add(raw.tag); const content = raw.contentText || raw.text; if (content) text.add(content); if (raw.attributes?.href) urls.add(raw.attributes.href); if (raw.tag === "form" || raw.attributes?.name) forms.add(`${raw.tag}:${raw.attributes?.name ?? ""}`); if (["button", "a", "input", "select", "textarea", "summary"].includes(raw.tag ?? "")) interactions.add(`${raw.tag}:${content ?? raw.attributes?.["aria-label"] ?? ""}`); for (const name of (raw.attributes?.class ?? "").split(/\s+/).filter(Boolean)) classes.add(name); } return { tags, text, urls, forms, interactions, classes }; }
