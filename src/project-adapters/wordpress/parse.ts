...
export async function parseWordPressProject(root: string, discovery: ProjectDiscoveryResult): Promise<SourceProject> {
  ...
  const contentExports = files.filter((file) => /(?:^|\/)(?:content|export|wordpress)[^/]*\.(?:json|xml)$/i.test(file.path) && file.path !== "wordpress-project.json").map((file) => ({ path: file.path, format: file.path.endsWith(".xml") ? "wxr" : "json", sha256: file.sha256 }));
  const inventoryFile = files.find((file) => file.path === "wordpress-project.json");
  const inventory = inventoryFile ? await Bun.file(join(root, inventoryFile.path)).json() as { version?: string; themeVersion?: string; plugins?: Record<string, string>; contentIds?: string[]; revision?: string } : undefined;
  const themeFiles = files.filter((file) => file.path === "theme.json" || file.path === "style.css" || file.path === "functions.php" || file.path.startsWith("templates/") || file.path.startsWith("parts/") || file.path.startsWith("patterns/")).map((file) => ({ path: file.path, sha256: file.sha256 }));
  return assembleSourceProject(discovery.contract, discovery.contractHash, { files, modules: [], roots, bindings: [], classVariants: [], styleSources, unresolved, metadata: { revision: discovery.contract.cms?.revision, capabilityHash: hashJson({ parser: "gen2prod-wordpress-blocks", version: discovery.contract.framework.parserVersion }), wordpressGraph, themeJson: themeJson ? { version: themeJson.version, settingsHash: hashJson(themeJson.settings ?? {}), stylesHash: hashJson(themeJson.styles ?? {}) } : null, phpEvidence, themeFiles, contentExports, pluginInventory: inventory ? { wordpressVersion: inventory.version ?? "unknown", themeVersion: inventory.themeVersion ?? "unknown", plugins: inventory.plugins ?? {}, contentIds: inventory.contentIds ?? [], declaredRevision: inventory.revision ?? null } : null } });
}
