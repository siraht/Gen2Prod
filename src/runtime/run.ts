...
export async function executeRun(options: RunOptions): Promise<RunResult> {
  ...
  if (compiled.plan.source.executableEvents.length) requiredActions.push({ id: "inline-interaction-contract-required", summary: "Reimplement inline behavior from explicit interaction contracts", detail: `${compiled.plan.source.executableEvents.length} inline event handler(s) were intentionally excluded from deterministic output. Preserve only approved behavior through typed interaction contracts and tested modules.`, blocking: false });
