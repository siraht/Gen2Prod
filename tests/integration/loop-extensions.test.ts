...
test("records accepted production runs as distillation trajectories", async () => {
  ...
  const run = await executeRun({ input: join(fixture, "fixture.corrupted.html"), cssPath: join(fixture, "corrupted.css"), tokenPath: join(fixture, "fixture.gold.tokens.json"), mode: "legacy-conversion", profile: "migration", capture: false, config, policy: defaultPolicy });
