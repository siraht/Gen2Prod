...
test("records repeatable browser, accessibility, and exact visual-target evidence", async () => {
  ...
  const artifactPath = join(root, "site-spec.json"); const targetJsonPath = join(root, "visual-target.json"); const resultsPath = join(root, "results.json"); const fakeBrowser = join(root, "unavailable-browser");
  await Promise.all([Bun.write(artifactPath, JSON.stringify(artifact)), Bun.write(targetJsonPath, JSON.stringify(target)), Bun.write(resultsPath, JSON.stringify(results)), Bun.write(fakeBrowser, "not a browser\n")]);
  const unavailable = Bun.spawn(["bun", "run", "src/cli.ts", "--json", "evidence", "capture", root, "--spec", artifactPath, "--visual-target", targetJsonPath, "--results", resultsPath, "--output", join(root, "unavailable-results.json"), "--browser-executable", fakeBrowser, "--viewport", "640", "--height", "480"], { cwd: process.cwd(), stdout: "pipe", stderr: "pipe" });
  expect(await unavailable.exited).toBe(1);
  const unavailableEnvelope = JSON.parse(await new Response(unavailable.stdout).text()) as { ok: boolean; warnings: string[] };
  expect(unavailableEnvelope.ok).toBeFalse();
  expect(unavailableEnvelope.warnings.join(" ")).toContain("Failed to launch");
